...
export class IncludeResolver {
    ...
    public resolve(
        ...
        if (path.isAbsolute(normalizedInclude)) {
            const absoluteResult = this.tryResolve(
                normalizedInclude,
                "absolute",
                normalizedInclude
            );
        ...
        if (fromPath) {
            ...
            const relativeResult = this.tryResolve(
                relativePath,
                "relative",
                normalizedInclude
            );
         ...
         */
        const projectResult = this.resolveFromProject(
            normalizedInclude
        );
         ...
         */
        const packagesResult = this.resolveFromPackages(
            normalizedInclude
        );
         ...
         *         └── Core.hlsl
         */
        const packageCacheResult =
            this.resolveFromPackageCache(normalizedInclude);
    ...
    private resolveFromPackageCache(
        includePath: string
    ): IncludeResolution | undefined {
        const root = this.projectRoot.getPath();
        ...
        const packageRelativePath =
            includePath.substring("Packages/".length);

        const separatorIndex =
            packageRelativePath.indexOf("/");
