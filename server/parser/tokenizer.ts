...
export class Tokenizer {
  ...
  private readToken(): Token | undefined {
    ...
    if (ONE_CHARACTER_OPERATORS.has(char)) {
      return this.readFixedLengthToken('operator', 1);
    }

    if (SYMBOLS.has(char)) {
      return this.readFixedLengthToken('symbol', 1);
    }

    return this.readFixedLengthToken('unknown', 1);
  }
  ...
  private readNumber(): Token {
    ...
    while (!this.isAtEnd()) {
      ...
      if (char === 'f' || char === 'F' || char === 'h' || char === 'H' || char === 'u' || char === 'U') {
        value += char;
        this.advance();
        break;
      }
  ...
  private peek(lookahead = 0): string | undefined {
